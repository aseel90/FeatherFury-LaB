(() => {
  'use strict';
  if (window.__FF_MAIN_MENU_V2__) return;
  ...
  function ensure() {
    ...
    let secondary = main.querySelector('.ff-secondary-menu-actions');
    if (!secondary) {
      secondary = document.createElement('div'); secondary.className = 'ff-secondary-menu-actions';
      const birds = document.createElement('button'); birds.type='button'; birds.className='ff-birds-btn';
      birds.append(icon('bird','ff-bird-icon'));
      const label=document.createElement('span'); label.className='ff-birds-label'; birds.append(label);
      birds.addEventListener('click',openShop); secondary.append(birds);
      const actions=main.querySelector('.start-actions-group'); actions ? actions.after(secondary) : main.append(secondary);
    }
    ...
    return {screen,main,carousel,dots,secondary};
  }
  ...
  function apply() {
    ...
    const birds=ui.secondary.querySelector('.ff-birds-btn'); if(birds){birds.querySelector('.ff-birds-label').textContent=lang==='ar'?'الطيور':'BIRDS';birds.setAttribute('aria-label',lang==='ar'?'اختيار الطائر':'Choose bird');}
  ...
  window.__FF_MAIN_MENU_V2__={apply,openShop};
})();
