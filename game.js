(() => {
  ...
  function installCharacterSkinRouter() {
    if (typeof originalDrawBirdSkin !== 'function' || typeof window.drawBirdSkin !== 'function') return false;
    if (window.__FF_CHARACTER_SKIN_ROUTER_V1__) return true;

    const redesignedDrawBirdSkin = window.drawBirdSkin;
    window.drawBirdSkin = function(ctx, skinKey, x, y, rotation, wingCycle, scale = 1, inFever = false) {
      ...
      if (key === 'classic' || key === 'falcon') {
        return redesignedDrawBirdSkin(ctx, key, x, y, rotation, wingCycle, scale, inFever);
      }
      return originalDrawBirdSkin(ctx, key, x, y, rotation, wingCycle, scale, inFever);
    };

    window.__FF_CHARACTER_SKIN_ROUTER_V1__ = true;
    if (window.game) {
  ...
  loadScript('patch-runner.js?v=3').then(ok => {
    ...
    loadScript('patch-manifest.js?v=19').then(manifestOk => {
      ...
      loadScript('hero-blue-effects-v1.js?v=2')
        .then(() => loadScript('fierce-falcon-v1.js?v=2'))
        .then(() => installCharacterSkinRouter());
    });
  });
})();
