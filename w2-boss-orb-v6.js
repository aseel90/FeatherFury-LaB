...
function install(){
  ...
  if (game.__w2BossOrbV6Installed) return true;
  ...
  function processCollectedOrbs(g,s){
    ...
    const busy=!!s.bolt||s.blocks.some(b=>b.state==='TARGETED'||b.state==='SHAKE'||b.state==='FALLING');
    const ready=s.blocks.filter(b=>b.state==='READY');
    const wantsIce=!busy&&ready.length&&g.bird.y<H()*.43;
    const target=wantsIce?ready.reduce((best,b)=>!best||Math.abs(b.x-g.boss.x)<Math.abs(best.x-g.boss.x)?b:best,null):null;
    s.lockedBlockId=target?.id??null;
    if(!target){s.charge=0;return;}
    target.hits=target.hits||0; s.charge=target.hits; s.required=ORBS_PER_DROP;
    for(const p of shots){
      if(!p?.active||p.__w2V6Consumed)continue;
      p.__w2IceTarget=target.id;
      const dx=target.x-p.x,dy=target.y-p.y,dist=Math.hypot(dx,dy)||1;
      p.vx=Math.max(10.8,Number(p.vx||15.5));
      p.vy=clamp(Number(p.vy||0)+(dy/dist)*.42,-7.4,7.4);
      if(dist<34){
        p.__w2V6Consumed=true;p.active=false;target.hits++;s.charge=target.hits;
        g.screenShake=Math.max(g.screenShake||0,4);g.sound?.playHit?.();
        for(let n=0;n<8;n++)g.particles.push({x:target.x+(Math.random()-.5)*28,y:target.y+(Math.random()-.5)*20,vx:(Math.random()-.5)*5,vy:(Math.random()-.5)*4,size:1.5+Math.random()*2,color:'#dbeafe',life:.48});
        if(target.hits>=ORBS_PER_DROP)triggerIceStrike(g,s,target);
        break;
      }
    }
  ...
  function impactBoss(g,s,block){
    ...
    const maxHp=Math.max(4,C.W2_BOSS_HP||8),iceDamage=Math.max(1,Math.ceil(maxHp*.22));
    g.boss.hp=Math.max(1,g.boss.hp-iceDamage);
    ...
    g.boss.state='V4_STUN';g.boss.timer=0;s.nextOrbIn=66;
  }
  ...
  game.__w2BossOrbV6Installed=true;console.log('[FF-LAB] w2-boss-orb-v6-installed');return true;
}
