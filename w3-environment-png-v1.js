...
const ready=(async()=>{
  ...
  function background(ctx,frame=0){
    if(!isW3(g))return oldBg?.(ctx,frame);
    const boss=bossFight(g),p2=phase2(g),w=W(),h=H(),gy=GY(),charged=boss||Number(g.score||0)>=Number(C.STAGE1_END||15);
    ctx.save();
  ...
  function ground(ctx,frame=0){
    if(!isW3(g))return oldGround?.(ctx,frame);
  ...
  window.__FF_W3_ENVIRONMENT_PNG_V1__={version:VERSION,procedural:true,animatedGround:true,parallaxGround:true,bossReactive:true,phase2Reactive:true,legacyStripes:false,finalOwner:true};
